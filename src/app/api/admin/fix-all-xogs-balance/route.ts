...
export async function POST() {
  try {
    ...
    console.log(`找到 ${users.length} 个用户，开始修复Xogs余额...`);
    ...
    for (const user of users) {
      ...
      updatedCount++;
      console.log(`用户 ${user.name || user.id} 余额修复: ${user.xogsBalance} → ${correctXogsBalance}`);
    ...
    return NextResponse.json({
      success: true,
      message: `修复完成！共修复了 ${updatedCount} 个用户的余额`,
    ...
    console.error('Error fixing all xogs balances:', error);
