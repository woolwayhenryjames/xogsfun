...
export async function POST() {
  try {
    ...
    return NextResponse.json({
      ...
      detailed: {
        baseCalculation: `AI评分 ${user.aiScore} × 10 = ${baseXogs}`,
        inviteCalculation: `邀请奖励 = ${totalInviteRewards}`,
        checkinCalculation: `签到奖励 = ${totalCheckinRewards}`,
        taskCalculation: `任务奖励 = ${totalTaskRewards}`,
        infofiCalculation: `InfoFi奖励 = ${totalInfofiRewards}`,
      }
    });

  } catch (error) {
    console.error('Error fixing user xogs balance:', error);
